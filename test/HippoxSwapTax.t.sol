...
contract HippoxSwapTaxTest is Test {
    ...
    function testDefaultFeeAndTax() public view {
        assertEq(
            HippoxSwapPair(pair).feeNumerator(),
            3,
            "default AMM fee = 0.3%"
        );
        assertEq(HippoxSwapPair(pair).taxBps(), 10, "default tax = 0.1%");
        assertEq(HippoxSwapPair(pair).creator(), creator, "creator set");
        assertEq(
            HippoxSwapPair(pair).admin(),
            creator,
    ...
    function testCreatorCanSwitchAdmin() public {
        vm.prank(creator);
        HippoxSwapPair(pair).setAdmin(newAdmin);
        assertEq(HippoxSwapPair(pair).admin(), newAdmin, "admin switched");
        ...
        vm.prank(newAdmin);
        HippoxSwapPair(pair).setFeeNumerator(5);
        assertEq(HippoxSwapPair(pair).feeNumerator(), 5);
    }
    function testOnlyCreatorCanSwitchAdmin() public {
        vm.prank(attacker);
        vm.expectRevert("ONLY_CREATOR");
        HippoxSwapPair(pair).setAdmin(attacker);
        vm.prank(newAdmin);
        vm.expectRevert("ONLY_CREATOR");
        HippoxSwapPair(pair).setAdmin(attacker);
    }
    function testCannotSetAdminToZero() public {
        ...
        HippoxSwapPair(pair).setAdmin(address(0));
    }
    ...
    function testSetFeeToMax() public {
        vm.prank(creator);
        HippoxSwapPair(pair).setFeeNumerator(10); // 1%
        assertEq(HippoxSwapPair(pair).feeNumerator(), 10);
    }
    function testSetFeeOverMaxReverts() public {
        vm.prank(creator);
        vm.expectRevert("FEE_TOO_HIGH");
        HippoxSwapPair(pair).setFeeNumerator(11);
    }
    function testSetFeeToZero() public {
        vm.prank(creator);
        HippoxSwapPair(pair).setFeeNumerator(0);
        assertEq(HippoxSwapPair(pair).feeNumerator(), 0);
    }
    ...
    function testZeroFeeZeroTaxGivesMaxOutput() public {
        vm.startPrank(creator);
        HippoxSwapPair(pair).setFeeNumerator(0);
        HippoxSwapPair(pair).setTaxBps(0);
        vm.stopPrank();
        address[] memory path = new address[](2);
        path[0] = address(tokenA);
        path[1] = address(tokenB);
    ...
    function testRapidFeeChanges() public {
        for (uint256 i = 0; i <= 10; i++) {
            vm.prank(creator);
            HippoxSwapPair(pair).setFeeNumerator(i);
            assertEq(HippoxSwapPair(pair).feeNumerator(), i);
        }
        vm.prank(creator);
        vm.expectRevert("FEE_TOO_HIGH");
        HippoxSwapPair(pair).setFeeNumerator(11);
    ...
    function testRapidTaxChanges() public {
        for (uint256 i = 0; i <= 100; i += 10) {
            vm.prank(creator);
            HippoxSwapPair(pair).setTaxBps(i);
            assertEq(HippoxSwapPair(pair).taxBps(), i);
        }
        vm.prank(creator);
        vm.expectRevert("TAX_TOO_HIGH");
        HippoxSwapPair(pair).setTaxBps(101);
    ...
    function testAdminTransferLocksOutOldAdmin() public {
        ...
        HippoxSwapPair(pair).setTaxBps(50);
        vm.expectRevert("ONLY_ADMIN");
        HippoxSwapPair(pair).setTaxRecipient(creator);
        vm.stopPrank();
        // New admin works.
        ...
        vm.stopPrank();
        assertEq(HippoxSwapPair(pair).feeNumerator(), 5);
        assertEq(HippoxSwapPair(pair).taxBps(), 50);
        assertEq(HippoxSwapPair(pair).taxRecipient(), taxSink);
    }
    ...
    function testTaxRecipientRejectingTokenRevertsSafely() public {
        ...
        vm.startPrank(creator);
        HippoxSwapPair(rtPair).setTaxBps(100); // 1%
        HippoxSwapPair(rtPair).setTaxRecipient(taxSink);
        vm.stopPrank();
        address[] memory path = new address[](2);
        path[0] = address(rt);
        path[1] = address(tokenB);
    ...
    function testTinyAmountTaxProportional() public {
        vm.startPrank(creator);
        HippoxSwapPair(pair).setTaxBps(10); // 0.1%
        HippoxSwapPair(pair).setTaxRecipient(taxSink);
        vm.stopPrank();
        address[] memory path = new address[](2);
        path[0] = address(tokenA);
    ...
    function testSwapWithOutputEqualToReserveReverts() public {
        (, uint112 r1) = HippoxSwapPair(pair).getReserves();
        vm.prank(creator);
        vm.expectRevert("INSUFFICIENT_LIQUIDITY");
        HippoxSwapPair(pair).swap(0, r1, creator);
    }
    ...
    function testFeeAtMaxThenExceedReverts() public {
        ...
        HippoxSwapPair(pair).setFeeNumerator(11);
        vm.stopPrank();
        assertEq(HippoxSwapPair(pair).feeNumerator(), 10, "fee stays at max");
    }
    ...
    function testTaxAtMaxThenExceedReverts() public {
        ...
        HippoxSwapPair(pair).setTaxBps(101);
        vm.stopPrank();
        assertEq(HippoxSwapPair(pair).taxBps(), 100, "tax stays at max");
    }
    ...
    function testMultiHopWithMaxFeeAndTax() public {
        ...
        HippoxSwapPair(pair).setFeeNumerator(10);
        HippoxSwapPair(pair).setTaxBps(100);
        HippoxSwapPair(pairBC).setFeeNumerator(10);
        HippoxSwapPair(pairBC).setTaxBps(100);
        vm.stopPrank();
